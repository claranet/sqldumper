...
for C in $CONTAINERS; do
    ...
    if [[ -z "${_DUMP_USERNAME}" || "${_DUMP_USERNAME}" == "null" ]]; then
        ...
        _DUMP_USERNAME=$(docker inspect $C |  jq -r '.[].Config.Env[] | select(. | contains("MYSQL_USER"))' | sed 's/MYSQL_USER=//g' | sed 's/[(^")|("$)]//g')
    fi
    ...
    if [[ -z "${_DUMP_PASSWORD}" || "${_DUMP_PASSWORD}" == "null" ]]; then
        ...
        _DUMP_PASSWORD=$(docker inspect $C |  jq -r '.[].Config.Env[] | select(. | contains("MYSQL_PASSWORD"))' | sed 's/MYSQL_PASSWORD=//g' | sed 's/[(^")|("$)]//g')
    fi
    ...
    if [[ -z "${_DUMP_DATABASE}" || "${_DUMP_DATABASE}" == "null" ]]; then
        ...
        _DUMP_DATABASE=$(docker inspect $C |  jq -r '.[].Config.Env[] | select(. | contains("MYSQL_DATABASE"))' | sed 's/MYSQL_DATABASE=//g' | sed 's/[(^")|("$)]//g')
    fi
